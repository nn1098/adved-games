...
bool LoadObj(std::vector<shape_t> &shapes,       // [output]
             ...
             std::string &err, const char *filename, const char *mtl_basepath, bool triangulate) {
  ...
  return LoadObj(shapes, materials, err, ifs, matFileReader, triangulate);
}
...
bool LoadObj(std::vector<shape_t> &shapes,       // [output]
             ...
             fileIO::Textfile tf, const char *mtl_basepath, bool triangulate) {
  shapes.clear();
  ...
  return LoadObj(shapes, materials, err, tf, matFileReader, triangulate);
}

bool LoadObj(std::vector<shape_t> &shapes,       // [output]
             std::vector<material_t> &materials, // [output]
             std::string &err,                   // [output]
             fileIO::Textfile tf, MaterialReader &readMatFn, bool triangulate) {
  std::vector<float> v;
  std::vector<float> vn;
  std::vector<float> vt;
  std::vector<unsigned int> vl;
  ...
  for (size_t line = 0; line < tf.lines; line++) {
    //  inStream.getline(&buf[0], maxchars);

    // std::string linebuf(&buf[0]);
    std::string linebuf = tf.GetLine(line);
